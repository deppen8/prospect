...
class Coverage:
    ...
    def from_shapefile(cls, area: Area, name: str, path: str, su_type: str, spacing: float, orient_axis: str = 'long', min_time_per_unit: float = 1):
        ...
        orientation = cls.optimize_orientation_by_area_orient(min_rect=min_rot_rect, axis=orient_axis)
    ...
    def make_transects(cls, area: Area, name: str, spacing: float = 10, sweep_width: float = 2, 
        ...
        lines_gs = cls.get_unit_bases(survey_unit_type='transect', area=area, centroid=centroid, spacing=spacing)

        if optimize_orient_by == 'area_coverage':  # set orientation to maximize area
            orientation = cls.optimize_orientation_by_area_coverage(lines_gs, centroid, area=area, buffer=sweep_width, increment=orient_increment)
    ...
    def make_radials(cls, area: Area, name: str, spacing: float = 10, radius: float = 2, 
        ...
        points_gs = cls.get_unit_bases(survey_unit_type='radial', area=area, centroid=centroid, spacing=spacing)

        if optimize_orient_by == 'area_coverage':  # set orientation to maximize area
            orientation = cls.optimize_orientation_by_area_coverage(points_gs, centroid, area=area, buffer=radius, increment=orient_increment)
        elif optimize_orient_by == 'area_orient':
            orientation = cls.optimize_orientation_by_area_orient(min_rect=min_rot_rect, axis=orient_axis)
    ...
    def get_unit_bases(survey_unit_type: str, area: Area, centroid: Point, spacing: float = 10) -> gpd.GeoSeries:
        ...
        # calculate x values
        xs = Coverage.coord_vals_from_centroid_val(centroid.x, n_transects, spacing)
        ...
        if survey_unit_type == 'transect':
            ...
            ys = Coverage.coord_vals_from_centroid_val(centroid.y, n_transects, spacing)
